...
def passenger_dashboard(user):
    ...
    with tab_share:
        ...
        if not shared_rides:
            st.info("No shared rides available nearby right now.")
        else:
            for ride in shared_rides:
                with st.expander(f"🚗 Ride from {ride['pickup']} to {ride['drop_loc']}"):
                    st.metric("Price", f"₹{ride['price']}")
                    if st.button("Book Seat", key=f"join_{ride['id']}"):
    ...
    # --- TAB D: PROFILE (UPDATED WITH BAR GRAPH) ---
    with tab_profile:
        ...
        conn = get_db_connection()
        # Fetching only Date and Price for completed rides
        graph_data = conn.execute(
            "SELECT created_at, price FROM rides WHERE (passenger=? OR passenger2=?) AND status!='Cancelled'",
        ...
        if graph_data:
            # 1. Create DataFrame
            df_graph = pd.DataFrame(graph_data, columns=["Date", "Price"])

            # 2. Convert string timestamp to just Date object
            df_graph["Date"] = pd.to_datetime(df_graph["Date"]).dt.date

            # 3. Group by Date and Sum the Price (Daily Spending)
            daily_spending = df_graph.groupby("Date")["Price"].sum()

            # 4. Display the Bar Chart
            st.bar_chart(daily_spending)
            st.caption("Total spending per day on CityCab")
        else:
            st.info("Book your first ride to see your spending analytics!")
